...
(function(root, factory) {
  ...
  var multirange = function(input) {
    ...
    input.value = isNaN(values[0]) ? (min + (max - min) / 2) : parseFloat(values[0]);
    ghost.value = isNaN(values[1]) ? (min + (max - min) / 2) : parseFloat(values[1]);
    ...
    if (descriptor.get) {
      ...
      Object.defineProperty(input, "value", {
        ...
        set: function(v) {
          var values = v.split(",");
          this.valueLow = parseFloat(values[0]);
          this.valueHigh = parseFloat(values[1]);
          update();
        },
        enumerable: true
      });
